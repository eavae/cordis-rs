...
async fn patch_overrides_group_inject_intercept_isolate() {
    async {
        let dir = std::env::temp_dir().join(format!("cordis-patch-gap-a-{}", std::process::id()));
...
async fn patch_writes_arbitrary_extra_key_and_round_trips() {
    async {
        let dir = std::env::temp_dir().join(format!("cordis-patch-gap-b-{}", std::process::id()));
        let mut patch = PatchOptions {
            id: Some("1".to_string()),
            ..Default::default()
        };
        ...
        wait_until(|| fs::read_to_string(&path).is_ok_and(|content| content.contains("custom: x")))
            .await;
...
async fn patch_targets_two_level_nested_entry() {
    async {
        let dir = std::env::temp_dir().join(format!("cordis-patch-gap-c-{}", std::process::id()));
        let fixture = r#"
- id: outer
...
async fn patch_inserts_into_nested_subgroup() {
    async {
        let dir = std::env::temp_dir().join(format!("cordis-patch-gap-d-{}", std::process::id()));
        let fixture = r#"
- id: outer
  name: "@cordisjs/plugin-group"
  group: true
...
async fn patch_null_clears_disabled_and_config() {
    async {
        let dir = std::env::temp_dir().join(format!("cordis-patch-gap-e-{}", std::process::id()));
        let patch = PatchOptions {
            id: Some("1".to_string()),
...
async fn patch_after_root_insert_matches_js_semantics() {
    async {
        let dir = std::env::temp_dir().join(format!("cordis-patch-gap-f-{}", std::process::id()));
...
async fn patch_name_mismatch_warns_and_skips() {
    async {
        let dir = std::env::temp_dir().join(format!("cordis-patch-gap-g-{}", std::process::id()));
        let patch = PatchOptions {
            id: Some("1".to_string()),
...
async fn patch_matching_name_applies() {
    async {
        let dir = std::env::temp_dir().join(format!("cordis-patch-gap-h-{}", std::process::id()));
        let patch = PatchOptions {
            id: Some("1".to_string()),
...
async fn patch_nonexistent_id_warns() {
    async {
        let dir = std::env::temp_dir().join(format!("cordis-patch-gap-i-{}", std::process::id()));
        let patch = PatchOptions {
...
async fn patch_insert_into_non_group_warns() {
    async {
        let dir = std::env::temp_dir().join(format!("cordis-patch-gap-j-{}", std::process::id()));
        let patch = PatchOptions {
            id: Some("1".to_string()),
            insert: Some(vec![EntryOptions {
                id: "extra".to_string(),
