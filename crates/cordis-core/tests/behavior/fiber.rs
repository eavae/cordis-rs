...
//! schedules fiber state-machine tasks as tokio tasks, so these tests
...
async fn fiber_inertia_lock_1() {
    super::with_timers(|timers| async move {
...
async fn wait_task_survives_panicking_background_task() {
    async {
        ...
        let handle = root
            ...
            .expect("effect");
        let result =
            tokio::time::timeout(std::time::Duration::from_millis(500), handle.wait_task()).await;
        assert!(
            result.is_ok(),
