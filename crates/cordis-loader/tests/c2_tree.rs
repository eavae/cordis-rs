...
async fn create_remove_and_write() {
    ...
    local
        .run_until(async {
            ...
            loader.tree_handle().write_callback.borrow_mut().replace({
                let writes = writes.clone();
                Rc::new(move || writes.set(writes.get() + 1))
            });
...
async fn resolve_path_and_errors() {
    ...
    local
        .run_until(async {
            ...
            assert!(Rc::ptr_eq(&resolved, &entry));
...
async fn get_tasks_and_await() {
    ...
    local
        .run_until(async {
            ...
            assert!(entry.fiber.borrow().is_some());
        })
        .await;
}
